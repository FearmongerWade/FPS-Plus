...
// || FlxG.keys.justPressed.X  || FlxG.keys.justPressed.C || FlxG.keys.justPressed.V
	...
	function updateHeads(?changedCharacters:Bool = false):Void{
		...
		if(changedCharacters){
			var leftChar:CharacterInfoBase = Type.createInstance(Type.resolveClass("characters.data." + player2DropDown.selectedLabel), []);
			var rightChar:CharacterInfoBase = Type.createInstance(Type.resolveClass("characters.data." + player1DropDown.selectedLabel), []);
	...
	public static function loadLists():Void{
		...
		var characterClasses = CompileTime.getAllClasses("characters.data", false, CharacterInfoBase);
		...
		var stageClasses = CompileTime.getAllClasses("stages.data", false, BaseStage);
